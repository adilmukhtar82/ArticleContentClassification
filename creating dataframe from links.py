...
def clean(cont):
    ...
    return cont
link_id = 1
df = pd.read_excel('CW All Links.xlsx', header=None)
pos_taggers_dict = {}
...
LANGUAGE = "english"
for i, k in zip(df[0].values, df[1].values):
    try:
        parser = HtmlParser.from_url(i, Tokenizer(LANGUAGE))
        content = ''

        for s in list(parser.document.sentences):
            content = (content + str(s)).lower()
        ...
        with open('NLP Preprocessed links Computer world\\' + str(spon) + '-' + str(link_id) + '.txt', 'w') as file:
            file.write(content)
...
df.to_csv('bag of POS CW.csv', sep=',',
          encoding='utf-8')